...
class T2DetFCOSHead(RotatedFCOSHead):
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            if self.use_circumiou_loss:
                ...
                # Targets and labels are OBBs directly, use rotated_iou loss
                loss_bbox = self.loss_bbox(
            ...
            if loss_bbox.item() < 0.4:
                ...
                # (bcnt is supposed to be 3, for ori, rot, and flp and sca)
                ...
                compacted_angle_preds = self.angle_coder.decode(
                    compacted_angle_preds.permute(1, 0, 2).contiguous().view(-1, 3), keepdim=False).view(-1, 2)
                ...
                if b_sca:
                    ...
                    pair_box_target = torch.empty(
                        *bid.shape, pos_decoded_bbox_targets.shape[-1],
                        device=bid.device).index_reduce_(
                        0, idx, pos_decoded_bbox_targets, 'mean',
                        include_self=False)[bmsk].view(-1, 2, pos_decoded_bbox_targets.shape[-1])
                    pair_box_preds = torch.empty(
                        *bid.shape, pos_decoded_bbox_preds.shape[-1],
                        ...
                        -1, 2, pos_decoded_bbox_preds.shape[-1])
                    ...
                    #         self.obb2xyxy(ori_box),
                    #         self.obb2xyxy(trs_box))

                    # 计算框的面积,用L1loss来计算面积一致性
                    ori_box_area = get_box_scales(ori_box)
                    trs_box_area = get_box_scales(trs_box)
                    if len(ori_box):
                        loss_scale_ss = self.loss_scale_ss(ori_box_area, trs_box_area)

                    # if len(ori_box):
                    #     loss_scale_ss = self.loss_scale_ss(ori_box, trs_box)
                    else:
                        loss_scale_ss = pos_decoded_bbox_preds.sum() * 0
                ...
                if self.use_reweighted_loss_bbox:
                    loss_symmetry_ss = math.exp(-loss_bbox) * loss_symmetry_ss
            ...
            loss_scale_ss = pos_bbox_preds.sum()
